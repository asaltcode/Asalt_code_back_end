...
const paidStatus = async (req, res) =>{  
    try {
        switch (req.body.status) {
            case "succeeded":
                await paymentModel.updateMany({order_id: req.body.orderDetails.orderId}, {$set: {
                    access: true,
                    paymentId: req.body.orderDetails.paymentId.current,
                    payment_status: req.body.status,
                    signature: req.body.orderDetails.signature
                }})
                const payment = await paymentModel.find({order_id: req.body.orderDetails.orderId})        
...
const courseAccess = async (req, res) =>{
    try {
        ...
        if(token){
            ...
            const payment = await paymentModel.findOne({$and: [{course_id: req.body.course_id}, {access: true}, {user_id: payload.id}]})
